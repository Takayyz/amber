...
- **Upload retries resume rather than restart**: an upload is three steps — presign, PUT to R2, then the `media_items` row. A retry after the PUT already succeeded writes only the row, reusing the object in R2. Starting over would upload a second copy and leave the first one referenced by nothing, and nothing purges R2 (permanent delete is out of scope). A failed PUT does start over, since a signed URL only lasts an hour and the refusal can be the signature itself.
