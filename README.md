...
- **Search**: tag-based filtering searches across the entire service, not scoped to a single album — matches the flat, service-wide access model. Selecting multiple tags is OR logic (any match shows the photo), not AND — avoids the common "zero results" trap of over-narrowing. It gets its own route (`/search`) rather than a mode the album list drops into, since a result set spanning albums has no album to belong to, and the selected tags live in the URL (`/search?tags=家族&tags=旅行`) so a search can be shared or bookmarked the way a photo can. The parameter repeats rather than joining on a comma, because a tag is free text and may contain one — a joined list would then tear that tag in half on the way back out. The URL names the tags rather than identifying them by uuid, and so does the query behind it, which leaves nothing to resolve on the way in. Opening a photo from the results keeps the results as its context: the arrows step to the next match rather than to the next photo in whichever album that one happens to live in, and closing returns to the search rather than to that album. The tag list offers the whole vocabulary as toggles — seeing what tags exist is itself the way in, at the scale one household produces — and the box above it narrows that list rather than running a search of its own: selecting is still a matter of pressing a tag.
- **The way into search is a box, not a button**: the header carries a search field on every screen, and pressing it goes to `/search`, where the same field in the same place becomes the real one and takes focus. It is one route change rendered as none — the field is the same size to the pixel either side, which holds because the header pins its right-hand group and nothing there changes width between screens. A field that moved even slightly would read as a new screen, which is the thing being avoided. What it costs is that `/search` had to start using `AppHeader` like the other screens, and that the tag row needed a loading state: the screen used to say "まだタグがありません" until the vocabulary arrived, which is both untrue and exactly the flicker that gives the swap away. A phone keeps the old icon, since there is no room for a box between a back link and a name, and no illusion to buy — a tap on an icon was never going to read as focusing a field. There the field sits at the top of the search screen's own body instead.
- **What is typed narrows the tags, and lives in the URL**: `?q=` sits beside `?tags=`, so the field and the tag row read the same source without either one owning the other. Tags already selected stay on screen whatever is typed — filtering them out of sight would leave no way to see what the results are narrowed by, or to undo it — and "nothing matched" is tracked separately from "the list is empty", since with a tag or two still pinned there is a row on screen and a word that matched nothing would otherwise look like a word that did. Picking a tag drops `q`, putting the rest of the vocabulary back within reach for the next one. Escape clears the box, and clears the screen once there is nothing left to clear.
...
- **Palette**: the UI is built around the mark's own two colours rather than the shadcn defaults, which are greyscale. `--primary` is the amber itself, unshifted — `#B87514` on light, `#F0B04A` on dark — so a button and the logo above it are the same colour rather than nearly so. On light that forces `--primary-foreground` the other way round from the shadcn default: the amber is a mid-tone at oklch L 0.62, which carries white at only 3.76:1 and a near-black at 5.59:1, so the label on a solid button is dark. Dark mode needs no such inversion — the mark's dark variant is already L 0.80, light enough for the usual dark label. `--ring` follows `--primary`, since a grey focus halo around an amber control is the one place the old palette showed through.
- **The neutrals are warm, faintly**: every grey carries chroma 0.004–0.014 at the amber's own hue rather than sitting at zero. It is well under what registers as a colour, and the point is what it prevents — a greyscale UI with one amber button reads as a grey UI someone dropped a colour into, which is what the default palette gave. Backgrounds and cards stay equal to each other, as they were, so nothing gains a lift that was not there before.
- **Turquoise belongs to the invite flow**: `--invite` is the mark's accent arc, and the mark uses a second colour there because, in its own terms, the gaps in the enclosure are where someone new is let in and those people are not interchangeable. So the second colour is spent on exactly that — sending an invitation, and the invitations still in flight — rather than being handed to shadcn's `--accent`, which despite the name is the hover surface behind every menu row and would turn the whole UI green. It is applied at the call site through `tailwind-merge` rather than as a button variant, which keeps `ui/button.tsx` a file `shadcn add` can still overwrite.
- **Coloured actions are tints, not solid fills**: `default` was changed from upstream's solid `bg-primary` to a 10% tint, matching the `destructive` variant that was already written that way. The two stand next to each other on the album screen, and a 10% chip beside a 100% block read as two design languages rather than one — the rest of the light theme is near-white surfaces with hairline borders, so the solid was the only heavy element on the page and sat outside its own UI. The label is mixed 30% toward `--foreground`, which darkens it on light and lightens it on dark, because a tint that pale cannot carry `--primary` at its own lightness (3.70:1 against the page). That mix is `in oklab` rather than `in oklch` deliberately: oklch interpolates the hue angle, so mixing a colour toward the warm near-neutral `--foreground` rotates it — harmless for the amber, which is already at that hue, but it turned the turquoise invite button olive. The cost of a tint is a faint boundary: the chip itself sits at about 1.1:1 against the page, so what identifies it as a control is the colour of its label. The one place the amber still appears solid is the selected tag in search, where the weight is the point — it is a toggle marked out against identical unselected chips.
